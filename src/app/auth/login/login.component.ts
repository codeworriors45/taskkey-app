...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  showBackBtn = false;
  showLoginOptionsUI = true;
  showLoginWithPhoneNumberUI = false;
  showInCompleteUI = false;
  showLoginWithPhoneNumberStep1UI = false;
  showLoginWithEmailStep1UI = false;
  ...
  constructor(
    ...
    //this.window.windowRef.prompt('Please provide your email for confirmation')
    //this._tempFn()
    ...
    const index_US = this.countries.findIndex((c) => c.code== "US");
    
    if (index_US > -1) {
      const obj_US = {...this.countries[index_US]};
      this.countries.splice(index_US, 1);

      this.countries.splice( 0, 0, obj_US );
    }
    ...
    this.route
      ...
      .subscribe(params => {
        //debugger;
        if(params['emailLogin'] == 'success') {
          let email = params['email'];
          if(!params['email']) {
            email = this.window.windowRef.prompt('Please provide your email for confirmation');
          }
          console.log(email);
          //const email = params['email'] ? params['email'] : this.window.windowRef.prompt('Please provide your email for confirmation');
          this.authService.isSignInWithEmailLink().then((response) => {
            //debugger;
            console.log(response)
            if(response) {
              this.authService.signInWithEmailLink(email).then((result) => {
                //debugger;
                console.log('======================VALID LINK===========================');
                this.router.navigateByUrl('/auth/login');
              }).catch((error) => {
                console.log(error);
                alert('Invalid link or link expired!');
              });
            }
          })
  ...
  ngAfterViewInit() {
    //this.authService.initRecaptchaVerifier_login();
  }
  ...
  checkAndRedirectToHome() {
    //Check user login state manually or redirect if not logged-in -OR- create another Guard (Implement => canActivate)
    ...
    .subscribe(userData => {
      ...
      if(user) {
        if(user.publicKey) {
          console.log('Redirect to home from login => onLoad');
          ...
          user['publicKey'] = '';
          user['uuid'] = '';
          user['timeStamp'] = user.timeStamp ? user.timeStamp : moment().valueOf();
          this.ngFireUser = user;
          this.showBackBtn = false;
          this.showLoginOptionsUI = false;
          this.showLoginWithPhoneNumberUI = false;
          this.showInCompleteUI = true;
          this.manuallygetCryptoKey()
        }
      } else {
        const subs = this.afAuth.user.subscribe((user) => {
          ...
          if(user.uid) {
            const ngFireUser = {
              uid: user.uid,
              name: user.displayName ? (user.displayName.split(" ")[0] ? user.displayName.split(" ")[0] : '') : '',
      ...
      subscription.unsubscribe();
      console.log("Error: Login => constructor => getCurrentUser", error);
    });
  }

  setCountryCode(code) {
    this.signInForm.patchValue({countryCode: code.toString()})
  }

  manuallygetCryptoKey() {
    const getCryptoKey = this.ngFireFunctions.httpsCallable('getOrCreateCryptoKey');
    getCryptoKey({}).subscribe((result) => {
      const splitKey =  result.split('-');
      this.ngFireUser.publicKey = splitKey[10].trim();
    })
  }

  addOrUpdateFireStoreUser() {
    if(this.ngFireUser.publicKey) {
      const nameArr = [this.ngFireUser.name, this.ngFireUser.lastName]
      this.ngFireUser['displayName'] = nameArr.join(' ');
      this.ngFireUser['phoneNumber'] = this.ngFireUser.phone ? this.ngFireUser.phone : '';
      
      this.firebaseUserService.addOrUpdateUserData(this.ngFireUser).pipe(takeUntil(this.subscriptionDestroyed$)).subscribe(res => {
        console.log('Success, redirect to home');
        this.firebaseProjectService.loadAllUserData();
        this.router.navigateByUrl('/app/dashboard');
      }, err => console.log('Error: Apple Sign In => Add/Update user at FireStore', err));
    } else {
      const getCryptoKey = this.ngFireFunctions.httpsCallable('getOrCreateCryptoKey');
      getCryptoKey({}).subscribe((result) => {
        const splitKey =  result.split('-');
  ...
  googleSignIn() {
    this.authService.googleSignIn().then(response => {
      /*
      this.firebaseUserService.addOrUpdateUserData(response.user).pipe(takeUntil(this.subscriptionDestroyed$)).subscribe(res => {
        console.log('Success, redirect to home');
        //this.router.navigateByUrl('/app/dashboard');
        this.checkAndRedirectToHome();
      }, err => console.log('Error: Google Sign In => Add/Update user at FireStore', err));
      */
      this.checkAndRedirectToHome();
    }).catch(error => {
  ...
  appleSignIn() {
    /*
    this.authService.appleSignIn().then(response => {
      this.firebaseUserService.addOrUpdateUserData(response.user).pipe(takeUntil(this.subscriptionDestroyed$)).subscribe(res => {
        console.log('Success, redirect to home');
        //this.router.navigateByUrl('/app/dashboard');
        this.checkAndRedirectToHome();
      ...
      console.log('error at apple login: ', error);
    });
    */
    this.checkAndRedirectToHome();
  }
  ...
  sendMagicLink(email:string) {
    ...
    .then(() => {
      this.showBackBtn = false;
      this.showLoginWithEmailStep1UI = false;
    })
    .catch((error) => {
      console.log(error)
    });
  ...
  signInWithPhoneNumberStart() {
    this.showBackBtn = true;
    this.showLoginOptionsUI = false;
    this.showLoginWithPhoneNumberUI = true;
    this.showInCompleteUI = false;
    this.showLoginWithPhoneNumberStep1UI = true;
    this.showLoginWithEmailStep1UI = false;
  }

  signInWithMagicLinkStart() {
    this.showBackBtn = true;
    this.showLoginOptionsUI = false;
    this.showLoginWithPhoneNumberUI = false;
    this.showInCompleteUI = false;
    this.showLoginWithPhoneNumberStep1UI = false;
    this.showLoginWithEmailStep1UI = true;
  }

  signInWithPhoneNumber() {
    const fullNumber = '+' + this.signInForm.get('countryCode').value + this.signInForm.get('phoneNumber').value;
    this.authService.initRecaptchaVerifier_login();
    this.authService.signInWithPhoneNumber(fullNumber)
    .then((confirmationResult) => {
      ...
      this.authService.getRecaptchaVerifier().render().then((widgetId) => {
  ...
  onKeyDown(event) {
    ...
    if(allowedChars.indexOf(event.key) == -1 && event.key != 'Backspace') {
      event.preventDefault();
    }
  }

  onKeyUp(event: any, modelNo) {
    const allowedChars = '0123456789';
    if(event.key == 'Backspace' && modelNo > 1) {
      this[`field${modelNo-1}`].nativeElement.focus();
    } else if (modelNo < 6 && allowedChars.indexOf(event.key) != -1) {
      this[`field${modelNo+1}`].nativeElement.focus();
    } else if (modelNo == 6 && allowedChars.indexOf(event.key) != -1) {
      this[`field${modelNo}`].nativeElement.blur();
      this.verifyLoginCode();
    } else if (modelNo == 6 && event.key == "Enter") {
      this[`field${modelNo}`].nativeElement.blur();
      this.verifyLoginCode();
    }
  }
  ...
  verifyLoginCode():void {
    ...
    .confirm(this.otpDigit1 + this.otpDigit2 + this.otpDigit3 + this.otpDigit4 + this.otpDigit5 + this.otpDigit6)
    .then(result => {
      console.log('Success, redirect to home');
      //this.router.navigateByUrl('/app/dashboard');
      this.checkAndRedirectToHome();
      //this.router.navigate(['/app/dashboard'], { queryParams: { popup: 'setting' } });
