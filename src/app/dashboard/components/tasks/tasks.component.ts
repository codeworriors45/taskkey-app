...
export class TasksComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('fullImgModal', { static: false }) fullImgModal: ModalDirective;
  @ViewChild('taskParticipantsModal', { static: false }) taskParticipantsModal: ModalDirective;
  ...
  loadData() {
    ...
    this.getParticipantSubs = this.store.select(CardsState.getProjectById(this.projectId)).subscribe(async (card) => {
      this.card = card;
      const participants = [];
      for (let participant of card.participants) {
        ...
        const mappedUser = { ...user, ...participant };
        participants.push(mappedUser);
      };
    ...
    this.loadDataSubs = this.firebaseTaskService.getAndListenAllTaskById(this.projectId).subscribe(async (tasks) => {
      const mappedTasks = [];
      for (let task of tasks) {
        ...
        for (const participantId in task.participants) {
          const user: any = await this.firebaseProjectService.getUserById_N_addOrUpdateUserGloblly(participantId);
          const mappedUser = { ...user, ...task.participants[participantId] }
          participantsArr.push(mappedUser);
        }
        ...
        const createdBy: any = await this.firebaseProjectService.getUserById_N_addOrUpdateUserGloblly(task.createdBy)
        task._createdBy = createdBy.fullName;

        const mappedTask = { ...task, status, unreadMsg, participants: participantsArr, dueDate, statusForSorting, timestamp: miliseconds };
        //console.log(JSON.stringify(mappedTask))
      ...
      if (this.selectedTask) {
        ...
        this.tasks.forEach(task => {
          if (task.id == this.selectedTask.id) {
            ...
            this.selectedTask.participants = task.participants;
  ...
  getComments() {
    ...
    if (this.selectedTask) {
      ...
      this.commentSubs = this.ngFirestore.collection('projectsTasks').doc(this.selectedTask.id).collection('comments').snapshotChanges().subscribe(async (commentsData) => {
        ...
        for (let comment of commentsData) {
          ...
          if (comment.payload.doc) {
              ...
              for (const replyId in data.replies) {
                ...
                if (reply.comment) {
                  ...
                  if (this.rsaDecryptKey) {
                    ...
                    //decryptedComment = decryptedComment +' '+decryptedComment;
                  ...
                  //const parentCommentUsername = await this.getParentCommentUsername(reply.parentCommentId, reply.repliedCommentId);
                  const parentCommentUsername = '';
                  const user: any = await this.firebaseProjectService.getUserById_N_addOrUpdateUserGloblly(reply.userId)
                  const uProfileImage = user.profileImage;
                  ...
                  replies.push(reply);
                } else if (reply.commentType == 2) {
                  reply.trustedThumbUrl = this.sanitizer.bypassSecurityTrustUrl('data:image/png;base64,' + reply.imgThumb)
                }
              }
            }
            ...
            if (data.comment) {
              ...
              data.trustedThumbUrl = this.sanitizer.bypassSecurityTrustUrl('data:image/png;base64,' + data.imgThumb)
            }
            ...
            const user: any = await this.firebaseProjectService.getUserById_N_addOrUpdateUserGloblly(data.userId);
            const uProfileImage = user && user.profileImage;
            comments.push({ ...data, id, userName: user.name, uProfileImage, replies, firebaseReplies: data.replies, timestamp, timestampMiliseconds: miliseconds, comment: decryptedComment });
          }
        };
        ...
        for (let c of this.comments) {
          for (let reply of c.replies) {
            const _parentCommentUsername = await this.getParentCommentUsername(reply.parentCommentId, reply.repliedCommentId);
  ...
  showFullImage(comment) {
    this.modalImgSrc = comment.imgUrl;
    this.fullImgModal.show();
  }

  hideImgModal(): void {
    this.fullImgModal.hide();
  }
  ...
  async mapParticipantNamesAndCreateMessageStr(events) {
    ...
    for (let event of events) {
      const user: any = await this.firebaseProjectService.getUserById_N_addOrUpdateUserGloblly(event.userId);
  ...
  onKeyUp(event: any) {
    ...
    //console.log('onKeyUp', event.shiftKey, event.key);
  ...
  showParticipants() {
    //this.selectedTask.participants
  ...
  addPArticipantInMentioned_N_ShowNameInComment(participant) {
    this.showParticipantsMention = false;
    //document.execCommand('bold');
    if (this.mentionedParticipants.indexOf(participant.id) == -1) {
      this.mentionedParticipants.push(participant.id);
    }
    
    let previousComment = this.commentInput.nativeElement.innerHTML;
    if(previousComment.indexOf('@</span>') == previousComment.length - 8) {
      previousComment = previousComment.slice(0, previousComment.length - 8);
      this.commentInput.nativeElement.innerHTML = `${previousComment}</span><span id="i_${participant.id}">@${participant.name}</span>&nbsp;`;
    } else {
      previousComment = previousComment.slice(0, previousComment.length - 1);
      this.commentInput.nativeElement.innerHTML = `${previousComment}<span id="i_${participant.id}">@${participant.name}</span>&nbsp;`;
    }
    
    //this.commentInput.nativeElement.innerHTML = `${previousComment}@${participant.name}&nbsp;`;

    /*let previousComment = this.commentInput.nativeElement.innerText;
    previousComment = previousComment.slice(0, previousComment.length - 1);
    this.commentInput.nativeElement.innerHTML = `${previousComment}@${participant.name}&nbsp;`;*/
    ...
    setTimeout(() => {
      ...
      //this.setCaretAtStartEnd(this.commentInput.nativeElement, true);
      this.placeCaretAtEnd(this.commentInput.nativeElement)
    }, 0)
  }
  ...
  onKeyDown(e) {
    ...
    //console.log('onKeyDown', e.shiftKey, e.key)
  ...
  async _addComment(projectId, taskId) {
    const commentElement = this.commentInput.nativeElement;
    console.clear()
    //console.log(commentElement)
    
    this.mentionedParticipants.forEach(p_id => {
      const user = this.selectedTask.participants.find(_participant => _participant.id == p_id);
      const spanElRf = commentElement.querySelectorAll(`#i_${p_id}`);
      spanElRf.forEach(element => {
        //let element_innerHTML = element.innerHTML;
    ...
    const comment = this.commentInput.nativeElement.innerText.trim();
    console.log(comment)
  }
  ...
  async addTodo() {
    ...
    const userRole = this.checkUserIsAdminOrParticipantOrUnassigned(this.currentUser.userId, this.selectedTask);
    if (userRole != 'admin' && userRole != 'participant' && userRole != 'not_assigned') {
      alert("Only card's admin or task's participant can add ToDo on this task!")
      return;
    }
  ...
  updateToDoStatus(toDo) {
    ...
    const userRole = this.checkUserIsAdminOrParticipantOrUnassigned(this.currentUser.userId, this.selectedTask);
    if (userRole != 'admin' && userRole != 'participant' && userRole != 'not_assigned') {
      alert("Only card's admin or task's participant can update ToDo on this task!")
      return;
    }
  ...
  async getParentCommentUsername(parentCommentId, repliedCommentId) {
    if (parentCommentId == repliedCommentId) {
      const comment = this.comments.find(comment => comment.id == parentCommentId);
      const user: any = await this.firebaseProjectService.getUserById_N_addOrUpdateUserGloblly(comment.userId);
      return user.name;
    } else {
      const comment = this.comments.find(comment => comment.id == parentCommentId);
    ...
    if (ids) {
      for (let userId of ids) {
        ...
        if (userId) {
          ...
          comment = comment.replace(re, `<span>@${user.name ? user.name : userId}</span>`);
        }
      }
    }
  ...
  checkUserIsAdminOrParticipantOrUnassigned(userId, task) {
    const card = this.card;
    if (this.isCardAdmin(userId)) {
      return 'admin';
    }
  ...
  isCardAdmin(userId) {
    let isAdmin = false;
    this.card.participants.forEach(participant => {
      if (participant.id == userId && participant.admin) {
        isAdmin = true
      }
    });
    return isAdmin;
  ...
  isAdminOrCreator(userId, task) {
    if (this.isCardAdmin(userId) || task.createdBy == userId) {
      return true;
    }
    return false;
  }
  ...
  async addOrUpdateTask() {
    ...
    if (formValues.id) {
      taskDocRef.update(data).then(() => {
        ...
        const userIds_removedParticipants = [];
        this.taskObj_beforeUpdate.participants.forEach(participant => {
          const user = data.participants[participant.id];
          if (!user) {
            userIds_removedParticipants.push(participant.id)
          }
        });
        if (userIds_removedParticipants.length > 0) {
          this.saveTimeline(data.id, 'Task Action', TIMELINE_IDENTIFIER.TASK_MEMBER_UNASSIGN, userIds_removedParticipants.join(',')).subscribe(res => { });
        }

        const userIds_assignedParticipants = [];
        Object.keys(data.participants).forEach((pId) => {
          const user = this.taskObj_beforeUpdate.participants.find(p => p.id == pId);
          if (!user) {
            userIds_assignedParticipants.push(pId);
          }
        });
        if (userIds_assignedParticipants.length > 0) {
          this.saveTimeline(data.id, 'Task Action', TIMELINE_IDENTIFIER.TASK_MEMBER_ASSIGN, userIds_assignedParticipants.join(',')).subscribe(res => { });
        }
  ...
  showTaskParticipantsPopup(task) {
    console.clear()
    console.log(task)
    this.taskParticipantsModal.show();
  }

  hideTaskParticipantModal() {
    this.taskParticipantsModal.hide();
  }
